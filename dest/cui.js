...
(function($) {
    ...
    var adsConfigs = {
        menu: {
            url: "/1014900/D_DPPActive_728x90_ATF",
            size: [728, 90],
            id: "div-gpt-ad-1443035069060-0"
        },
        banner: {
            url: '/1014900/D_DPP_728x90_BTF2',
            size: [728, 90],
            id: 'div-gpt-ad-1453509014964-0'
        },
        menusold: {
            url: "/1014900/D_DPPInactive-Sold_728x90_ATF",
            size: [728, 90],
            id: "div-gpt-ad-1418863539592-0"
        },
        thankyou: {
            url: "/1014900/D_ThankYouPage",
            size: [430, 470],
            id: "div-gpt-ad-1443036412748-0"
        },
        dppaside: {
            url: "/1014900/D_DPPActive_300x250_ATF",
            size: [300, 250],
            id: "div-gpt-ad-1443034551899-0"
        },
        dppaside2: {
            url: "/1014900/D_DPPActive_300x250_BTF",
            size: [300, 250],
            id: "div-gpt-ad-1443034966503-0"
        },
        dppsoldaside: {
            url: "/1014900/D_DPPInactive-Sold_300x250_ATF",
            size: [300, 250],
            id: "div-gpt-ad-1443035492336-0"
        },
        dppsoldaside2: {
            url: "/1014900/D_DPPInactive-Sold_300x250_BTF",
            size: [300, 250],
            id: "div-gpt-ad-1443035637720-0"
        },
        articlerighttop: {
            url: "/1014900/D_Insider_300x250_ATF",
            size: [300, 250],
            id: "div-gpt-ad-1418864008982-0"
        },
        articlerightbottom: {
            url: "/1014900/D_Insider_300x600_BTF",
            size: [300, 600],
            id: "div-gpt-ad-1420764772184-0"
        },
        articlebottom: {
            url: "/1014900/D_Insider_300x250_BTF",
            size: [300, 250],
            id: "div-gpt-ad-1417741284563-0"
        },
        articleleft: {
            url: "/1014900/D_Insider_160x600_Scrolling",
            size: [160, 600],
            id: "div-gpt-ad-1421851206791-0"
        },
        allphoto: {
            url: "/1014900/D_AllPhotos_300x250_BTF",
            size: [300, 250],
            id: "div-gpt-ad-1443035262759-0"
        },
        dsp: {
            url: "/1014900/D_Schools_300x250_ATF",
            size: [300, 250],
            id: "div-gpt-ad-1443035826533-0"
        }
    };

    var adCode = context.adKeyWord;

    var googletag;

    var loadAdsScript = function() {
        if (!googletag) {
            googletag = window.googletag = googletag || {};
            googletag.cmd = googletag.cmd || [];

            (function() {
                var useSSL = "https:" == document.location.protocol;
                var adsUrl = (useSSL ? "https:" : "http:") + "//www.googletagservices.com/tag/js/gpt.js";
                Movoto.LoadJS(adsUrl, function() {
                    adsScriptLoaded = true;
                    $(document).trigger('initAds');
                });
            })();
        }
    };

    $.fn.ads = function(option) {
        var $this = $(this);
        var defaultOpt = {
            type: "",
        };
        var opt = $.extend(defaultOpt, option);
        var adsConfig = adsConfigs[opt.type];
        var _loadAds = function() {
            var pubads = googletag.pubads();
            googletag.defineSlot(adsConfig.url, adsConfig.size, adsConfig.id).addService(pubads);
            if (adCode) {
                pubads.setTargeting('DPPCounty', adCode);
            }
            pubads.enableSingleRequest();
            pubads.setTargeting();
            googletag.enableServices();
            googletag.display(adsConfig.id);
        };
        var initalAds = function() {
            $this.attr("id", adsConfig.id);
            $this.css({
                width: adsConfig.size[0] + "px",
                height: adsConfig.size[1] + "px",
                position: "relative",
                zIndex: 1,
            });
        ...
        if (adsScriptLoaded) {
            ...
            loadAdsScript();
            $(document).on('initAds', function() {
                initalAds();
            });
        }
        $this.attr('role', 'Advertising');
    };
    var loadAdsTimer = null;
    $(document).on("dom.load.ads", function() {
        if (loadAdsTimer) {
            clearTimeout(loadAdsTimer);
        }
        loadAdsTimer = setTimeout(function() {
            $("[data-ads]").each(function(index, item) {
                $(item).ads({
                    type: $(item).attr("data-ads")
                });
                $(item).removeAttr('data-ads');
...
(function($) {
    ...
    $.fn.chart = function(option) {
        if (!highchartLoaded && !$.fn.highcharts) {
            highchartLoaded = true;
            Movoto.LoadJS(
                context.staticUrl + 'javascripts/desktop/libs/jquery.highchart.js',
                function() {
                    $(document).trigger('highcharts.load');
                }
            );
        }
        var $this = $(this);
        if (option.data && option.data.length) {
            ...
            if (opt.title) {
                chartOpt.title = opt.title;
            }
            if (opt.exporting) {
                chartOpt.exporting = opt.exporting;
            }
            if (opt.subtitle) {
                chartOpt.subtitle = opt.subtitle
            }
            if (opt.tooltip) {
                chartOpt.tooltip = opt.tooltip;
            }
            chartOpt.xAxis = opt.keyOption;
            chartOpt.yAxis = opt.valueOption;
            switch (opt.type) {
                case $.chartType.pie:
                case $.chartType.col:
                case $.chartType.bar:
                    ...
                    for (var i = 0; i < opt.data.length; i++) {
                        if (opt.data[i].list && opt.data[i].list.length) {
                            var item = {
                    ...
                    break;
                case $.chartType.line:
                    chartOpt.series = opt.data.map(function(item) {
                        return {
                            name: item.name,
                            data: item.list
                        };
            ...
            if (!$.fn.highcharts) {
                $(document).one("highcharts.load", function() {
                    $this.highcharts(chartOpt);
                });
            } else {
                return $this.highcharts(chartOpt);
            }
        } else {
            return null;
        }
        $this.attr('role','Chart');
    };
    var inital = function() {
        $("[data-chart]").each(function() {
            var $this = $(this);
            var opt = $.call($this.attr('data-chart'), $this);
            $this.chart(opt);
            $this.removeAttr('data-chart');
        });
    };
    $(document).on('dom.load.chart', function() {
...
(function($) {
    ...
    $.loadGMap = function(callback, option) {
        ...
        var opt = $.extend({}, defaultOption, option);
        var mapUrl = 'http://maps.google.com/maps/api/js?v=3.3&libraries=geometry,places&' + context.googleMapKey + '&language=en-US';
        ...
        window.googleMapLoadCallBack = function() {
            if (opt.loadInfobox) {
                Movoto.LoadJS(context.staticUrl + 'javascripts/desktop/libs/google/infobox.js', function() {
                    mapLoaded = 2;
    ...
    $.fn.gSMap = function(option) {
        ...
        var reload = function() {
            ...
            if (opt.type == "streetview") {
                mapUrl = '/maps/api/streetview?';
                if (opt.lat && opt.lng) {
                    mapUrl += 'location=' + opt.lat + ',' + opt.lng;
                }
            } else {
                ...
                if (opt.markers && opt.markers.length > 0) {
                    mapUrl += '&markers=';
                    if (opt.icon) {
                        mapUrl += 'icon:' + encodeURIComponent(opt.icon) + '%7C';
                    }
                    var encodeMarker = opt.markers.map(function(e) {
                        return encodeURIComponent(e);
                    });
                    mapUrl += encodeMarker.join("%7C");
                }
            }
            mapUrl += '&size=' + $this.width() + 'x' + $this.height();
            mapUrl += '&' + context.googleMapKey;

            if (context.isDevelopment) {
                $this.attr('src', 'http://maps.googleapis.com' + mapUrl);
            } else {
                var data = {
                    url: mapUrl
                };
                $.post(context.appUrl + 'webservices/getgooglemapurlwithsignature', data).done(function(data) {
        ...
        $this.attr('role', 'GStaticMap');
        reload();
    };
    var inital = function() {
        $("[data-gsmap]").each(function() {
            var $this = $(this);
            if ($this.attr('data-gsmap')) {
                var center = $this.attr('data-gsmap').split(',');
                if (center.length == 2) {
                    var opt = {
                        lat: center[0],
                        lng: center[1],
                        ...
                        markers: $this.attr('data-markers') ? $this.attr('data-markers').split('|') : []
                    };
                    $this.gSMap(opt);
                }
            }
...
(function($) {
    ...
    var getIcon = function(type) {
        ...
        switch (type) {
            case iconType.home:
                icon.url += 'images/desktop/pin/homeicon.png';
                break;
            case iconType.school:
                icon.url += 'images/desktop/pin/schoolicon.png';
                break;
            case iconType.schoolred:
                icon.url += 'images/desktop/pin/red_school.png';
                break;
            case iconType.heart:
                icon.url += 'images/desktop/pin/favorite.png';
                break;
            case iconType.condored:
                icon.url += 'images/desktop/pin/red_condo.png';
                break;
            case iconType.condoblue:
                icon.url += 'images/desktop/pin/blue_condo.png';
                break;
            case iconType.condogreen:
                icon.url += 'images/desktop/pin/green_condo.png';
                break;
            case iconType.condogray:
                icon.url += 'images/desktop/pin/black_condo.png';
                break;
            case iconType.condoyellow:
                icon.url += 'images/desktop/pin/yellow_condo.png';
                break;
            case iconType.singlered:
                icon.url += 'images/desktop/pin/red_single.png';
                break;
            case iconType.singleblue:
                icon.url += 'images/desktop/pin/blue_single.png';
                break;
            case iconType.singlegreen:
                icon.url += 'images/desktop/pin/green_single.png';
                break;
            case iconType.singlegray:
                icon.url += 'images/desktop/pin/black_single.png';
                break;
            case iconType.singleyellow:
                icon.url += 'images/desktop/pin/yellow_single.png';
                break;
            case iconType.poiBank:
                icon.url += 'images/desktop/pin/poi-bank.png';
                break;
            case iconType.poiWorship:
                icon.url += 'images/desktop/pin/poi-worship.png';
                break;
            case iconType.poiRecreation:
                icon.url += 'images/desktop/pin/poi-recreation.png';
                break;
            case iconType.poiTransportHub:
                icon.url += 'images/desktop/pin/poi-train.png';
                break;
            case iconType.poiGas:
                icon.url += 'images/desktop/pin/poi-gas.png';
                break;
            case iconType.poiTexi:
                icon.url += 'images/desktop/pin/poi-taxi.png';
                break;
            case iconType.poiTransportation:
                icon.url += 'images/desktop/pin/poi-transportation.png';
                break;
            case iconType.poiGrocery:
                icon.url += 'images/desktop/pin/poi-grocery.png';
                break;
            case iconType.poiCafe:
                icon.url += 'images/desktop/pin/poi-cafe.png';
                break;
            case iconType.poiRestaurant:
                icon.url += 'images/desktop/pin/poi-restaurant.png';
                break;
            case iconType.poiBar:
                icon.url += 'images/desktop/pin/poi-bar.png';
                break;
            case iconType.poiShopping:
                icon.url += 'images/desktop/pin/poi-shopping.png';
                break;
        }
        return icon;
    };
    ...
    $.fn.gMap = function(option) {
        ...
        if (opt.draggable) {
            ...
            if (opt.onDraged) {
                ...
                google.maps.event.addListener(map, 'dragend', function() {
                    ...
                    // if (Math.abs(currentCenter.lat() - tempCenter.lat()) > 0.05 || Math.abs(currentCenter.lng() - tempCenter.lng()) > 0.05) {
                    //     opt.onDraged();
                    //     tempCenter = currentCenter;
                    // }
                    if (!opt.distanceControl || (opt.distanceControl && namespace.overDistance(tempCenter, currentCenter))) {
                        opt.onDraged();
                        tempCenter = currentCenter;
                    }
                });
        ...
        if (opt.zoomable && opt.onZoom) {
            ...
            google.maps.event.addListenerOnce(map, 'tilesloaded', function() {
                ...
                google.maps.event.addListener(map, 'zoom_changed', function() {
                    ...
                    timer = setTimeout(function() {
                        if (namespace.isAutoZoom) {
                            namespace.isAutoZoom = false;
                            // for first zoom bug
                            // if (initialing) {
                            //     initialing = false;
                            //     opt.onZoom();
        ...
        namespace.hasStreetView = function(callback) {
            var streetViewLocation = new google.maps.LatLng(opt.lat, opt.lng);
        ...
        namespace.showStreetView = function() {
            if (panorama) {
                ...
                sv.getPanoramaByLocation(streetViewLocation, 50, function(data, status) {
                    if (status == 'OK') {
                        ...
                        panorama = new google.maps.StreetViewPanorama($this.get(0), panoramaOptions);
                        map.setStreetView(panorama);
                    } else {
                        //if no street view set it as false.
        ...
        namespace.hideStreetView = function(option) {
            if (panorama) {
                panorama.setVisible(false);
            }
        };
        namespace.addMarker = function(option) {
            var defaultOption = {
                lat: 0,
                lng: 0,
                draggable: false,
            ...
            var opt = $.extend({}, defaultOption, option);
            var addedMark = namespace.getMarkerById(opt.id);
            if (addedMark) {
                return addedMark;
            }
            ...
            //inital events of marker
            if (opt.onClick) {
                google.maps.event.addListener(marker, 'click', function(e) {
                    //return false, will stop to pop 'click' event
                    return opt.onClick(namespace.map, marker, e);
                });
            }
            markers.push(marker);
            return marker;
        };
        namespace.getMarkerById = function(id) {
        ...
        namespace.setAllMap = function(map) {
            for (var i = 0; i < markers.length; i++) {
                markers[i].setMap(map);
            }
        };
        namespace.hideMarkers = function() {
            namespace.setAllMap(null);
        };
        namespace.showMarkers = function() {
            namespace.setAllMap(map);
        };
        namespace.deleteMarkers = function() {
            namespace.hideMarkers();
            markers = [];
        };
        namespace.getBounds = function() {
            return map.getBounds();
        };
        namespace.fitBounds = function(poiList) {
            var list = [];
            ...
            namespace.isAutoZoom = true;
            if (list && list.length) {
                var bounds = new google.maps.LatLngBounds();
                for (var i = 0; i < list.length; i++) {
                    if (list[i].lat && list[i].lng) {
                        bounds.extend(new google.maps.LatLng(list[i].lat, list[i].lng));
                    }
                }
        ...
        namespace.overDistance = function(cC, lC) {
            zoomLevel = map.getZoom();
            p1 = cC; // current center
            p2 = lC; // last center
            rad = function(x) {
        ...
        if (opt.streetView !== false) {
            namespace.showStreetView();
        }
        $this.data('gMap', namespace);
        $this.attr('role', 'GMap');
        return namespace;
    };
    var inital = function() {
        $('[data-gmap]').each(function() {
            var $this = $(this);
            if ($this.attr('data-gmap')) {
                var center = $this.attr('data-gmap').split(',');
                var callback = $this.attr('data-callback');
                if (center.length == 2) {
                    var opt = {
                        lat: center[0],
                        lng: center[1],
                        zoom: parseInt($this.attr('data-zoom')) || undefined,
                        type: $this.attr('data-type'),
                        streetView: $this.attr('data-streetView'),
                        streetViewControl: $this.attr('data-streetViewControl'),
                        panControl: $this.attr('data-panControl'),
                        zoomControl: $this.attr('data-zoomControl'),
                        mapTypeControl: $this.attr('data-mapTypeControl'),
                        autoResie: $this.attr('data-autoResie')
                    };
                    var map = $(this).gMap(opt);

                    if ($this.attr('data-markers')) {
                        var icon = $this.attr('data-icon');
                        var markers = $this.attr('data-markers').split('|');
                    ...
                    if (callback) {
                        $(document).trigger(callback, [$this]);
                    }
                }
            }
        });
...
//map pin
(function($) {
    $.fn.mapPin = function(option) {
        var defaultOption = {
            onclick: 'mappin.click',
            onload: 'mappin.load',
            pinTmp: '/tpls/search/components/desktop/mapview/views/mappin.ejs'
        };
        ...
        var googleMap = gmap.map;
        var markers = [],
            markerIndex = {};
        var $activePopover, $activePin, $hoverPin;
        var namespace = {

            // todo: module is better to initialize by itself.
            //      For this case, mapoverlay module is depended on googlemap,
            //      so currently we init this module in mapaview module.
            //
            // issue:
            //      if mapoverlay module is used by other modules other than mapview module(here is card module) before inilialized,
            //      script exception may happen.
            //
            // suggestions:
            //      1. self initialize
            //      2. deal with googlemap in self module
            //      3. make sure interface not report script exceptions
            //

            bindMarkers: function(fitBounds, houseList, matches) {
                namespace.removeMarkers();
                if (houseList && houseList.length) {
                    $markersOverlay = $('<div id="barqipin"></div>');

                    var houseDic = {};
                    for (var i = houseList.length - 1; i >= 0; i--) {
                        var house = houseList[i];
                        if (isCoordinateInvalid(house.longitude) || isCoordinateInvalid(house.latitude)) {
                            // ?
                            continue;
                        }
                        if (!isValidLngLat(house.longitude, house.latitude)) {
                            continue;
                        }
                        var re = new RegExp('#?' + house.subPremise);
                        var trimAddr = house.address.replace(re, '').trim();
                        var key = '' + house.longitude + house.latitude + trimAddr;
                        houseDic[key] = houseDic[key] || [];
                        houseDic[key].push(house);
                    }

                    Object.keys(houseDic).forEach(function(key) {
                        var houses = houseDic[key];
                        var houseone = houses[0];

                        var $pin = $(getHousePin(houses, matches));
                        $markersOverlay.append($pin);
                        var marker = {
                            lat: houseone.latitude,
                            lng: houseone.longitude,
                            $pin: $pin,
                            // house: item
                            houses: houses
                        };
                        ...
                        houses.forEach(function(house) {
                            markerIndex[house.propertyId] = marker;
                        });
                        ...
                        $pin.data('marker', {
                            ...
                            lat: marker.lat,
                            lng: marker.lng,
                            houses: marker.houses
                        });
                        //bind click event on the pin
                        if (opt.onclick) {
                            $pin.find('.pin').click(function() {
                                if (opt.onclick && $.isFunction(opt.onclick)) {
                                    opt.onclick();
                                } else {
                                    $(document).trigger(opt.onclick, [$(this).closest('.marker')]);
                                }
                                return false;
                            });
                        }
                    ...
                    $overlay.append($markersOverlay);
                    // if (type != searchTypeEnum.homeroam) {
                    if (fitBounds) {
                        gmap.fitBounds(markers);
                    } else {
                        $(document).trigger('googlemapfitbounds');
                    }
            ...
            setMarkerViewed: function(key) {
                var marker = markerIndex[key];
                if (marker && marker.houses && marker.houses.length === 1)
                    marker.$pin.addClass('viewed');
            },
            setMarkerActive: function(key) {
                namespace.resetActiveMarker();
                var marker = markerIndex[key];
                if (marker) {
                    $activePin = marker.$pin;
                    $activePin.addClass('on');
                    namespace.reviseMapCenterForMarker(marker);
                }
            },
            resetActiveMarker: function() {
                if ($activePin) {
                    $activePin.removeClass('on');
                    $activePin = null;
                }
            },
            setMarkerHover: function(key) {
                var marker = markerIndex[key];
                if (marker && marker.$pin != $activePin) {
                    $hoverPin = marker.$pin;
                    $hoverPin.addClass('on');
                    namespace.reviseMapCenterForMarker(marker);
                }
            },
            resetHoverMarker: function(key) {
                if ($hoverPin) {
                    $hoverPin.removeClass('on');
                    $hoverPin = null;
                }
            },
            setFavMarker: function(key) {
                var marker = markerIndex[key];
                if (marker) {
                    marker.$pin.addClass('fav');
                }
            },
            resetFavMarker: function(key) {
                var marker = markerIndex[key];
                if (marker) {
                    var $pin = marker.$pin;
                    if ($pin.data("houses") && $pin.data("houses").some(function(house) {
                        return house.isFavorite;
                    })) {
                        $pin.addClass('fav');
                    } else {
                        marker.$pin.removeClass('fav');
                    }
                }
            },
            ...
            //Sometimes the markers are out of the map range, we need to revise the map center.
            reviseMapCenterForMarker: function(marker) {
                if (marker) {
                    var bounds = googleMap.getBounds();
                    if (!bounds)
                        return;
                    var proj = overlayInstance.getProjection();
                    if (!proj)
                        return;
                    var $pin = marker.$pin;
                    var topRight = proj.fromLatLngToDivPixel(bounds.getNorthEast());
                    var bottomLeft = proj.fromLatLngToDivPixel(bounds.getSouthWest());
                    var centerPos = proj.fromLatLngToDivPixel(googleMap.getCenter());
                    var marginLeft = -$pin.width() / 2;
                    var marginTop = -29;
                    var offsetY = 0,
                        offsetX = 0;
                    var offset = $pin.position();
                    var top = offset.top;
                    var left = offset.left;
                    if (top > bottomLeft.y) {
                        offsetY = top - bottomLeft.y;
                    } else if (top < topRight.y - marginTop) {
                        offsetY = top - topRight.y + marginTop;
                    }
                    if (left > topRight.x + marginLeft) {
                        offsetX = left - topRight.x - marginLeft;
                    } else if (left < bottomLeft.x - marginLeft) {
                        offsetX = left - bottomLeft.x + marginLeft;
                    }
                    if (offsetX != 0 || offsetY != 0) {
                        googleMap.panBy(offsetX, offsetY);
                        overlayInstance.draw();
                    }
                }
            },
            reviseMapCenterForPopover: function(popover) {
                if (popover) {
                    var bounds = googleMap.getBounds();
                    if (!bounds)
                        return;
                    ...
                    var width = $popover.width() + padding * 2;
                    if (top > bottomLeft.y) {
                        offsetY = top - bottomLeft.y;
                    } else if (top < topRight.y) {
                        var $refreshControl = $('.map-refresh-controls');
                        var topFrom = 5;
                        if ($refreshControl.length != 0) {
                            topFrom = $refreshControl.height() + ($refreshControl.offset().top - $('.menu-header').height()) + 5;
                        }
                        top = (offset.top - topFrom);
                        offsetY = top - topRight.y;
                    }
                    if (left + width > topRight.x) {
                        offsetX = left + width - topRight.x;
                    } else if (left < bottomLeft.x) {
                        offsetX = left - bottomLeft.x - 10;
                    }
                    if (offsetX != 0 || offsetY != 0) {
                        googleMap.panBy(offsetX, offsetY);
                        overlayInstance.draw();
        ...
        //Create the Html marker by extend google.maps.OverlayView.
        function mapOverlayView(map) {
            var that = this;
            this.setMap(map);
            $overlay = $('<div class="mapOverlay"></div>');

            this.onAdd = function() {
                var panes = this.getPanes();
                $(panes.overlayImage).append($overlay);
            };
            ...
            this.draw = function() {
                ...
                for (var i = 0; i < markers.length; i++) {
                    ...
                    if (isValidLngLat(marker.lng, marker.lat)) {
                        var itemLatLng = new google.maps.LatLng(marker.lat, marker.lng);
                        var point = this.getProjection().fromLatLngToDivPixel(itemLatLng);
                        if (point) {
                            marker.$pin.css('left', point.x + 'px').css('top', point.y + 'px');
                            marker.point = point;
                        }
                    }
                }
        ...
        function isCoordinateInvalid(i) {
            return i === 0 || i === undefined || i === null;
        }

        function isValidLngLat(lng, lat) {
            /*http://maps.googleapis.com/maps/api/geocode/json?address=hawaii&sensor=false
            HI:
            "northeast" : {
                  "lat" : 22.37,
                  "lng" : -154.48
           ...
           */
            var HILngLat = lng < -154 && lng > -161 && lat > 18 && lat < 23;
            var AKLngLat = (lng < -129 || lng > 172) && lat > 50 && lat < 72;
            var USLngLat = lng < -66 && lng > -125 && lat > 25 && lat < 50;
            return HILngLat || AKLngLat || USLngLat;
        }

        // function getHousePin(house, matches) {
        function getHousePin(houses, matches) {
            var issingle = houses.length === 1;
            var text = '';
            var model = {}
            if (issingle) {
                var house = houses[0];
                model.text = '$' + ($.format.friendlyPrice(house.priceRaw) || '');
                model.tag = house.label;
                model.viewed = !!house.viewed;
            } else {
                model.text = houses.length + " Units";
                model.tag = '';
                model.viewed = false;
            }
            ...
            if (issingle) {
                ...
                if (matches && matches.indexOf(house.propertyId) >= 0) {
                    $pin.addClass('matched');
                }
            } else {
                for (var i = 0; i < houses.length; i++) {
                    if (matches && matches.indexOf(houses[i].propertyId) >= 0) {
                        $pin.addClass('matched');
                        break;
                    }
                }
                $pin.data("houses", houses);
            }
        ...
        overlayInstance = new mapOverlayView(googleMap);

        // $(document).click(function() {
        //     namespace.resetActiveMarker();
        // });

        $overlay.data('mapPin', namespace);
        if (opt.onload && $.isFunction(opt.onload)) {
            opt.onload(namespace);
        } else {
            $(document).trigger(opt.onload, [namespace]);
        }
        return namespace;
    };
})(jQuery);
//map popover
(function($) {
    $.fn.mapPopover = function(option) {
        var $this = $(this);
        if ($this.data('mapPopover')) {
            return $this.data('mapPopover');
        }
        var defaultOption = {
            html: '',
            template: '<div class="mapcard"><div class="content"></div><div class="arrowback"></div><div class="arrow"></div></div>',
            onload: 'mappopover.load',
            autoClose: true,
            autoPosition: true,
            target: $this.closest('.mapOverlay'),
            theme: null
        };
        var opt = $.extend({}, defaultOption, option);
        var $mapPopover = null;
        ...
        function initalPopover() {
            $mapPopover = $(opt.template);
            $content = $(opt.html);
            $mapPopover.find('.content').append($content);
            if (opt.theme) {
                $mapPopover.addClass(opt.theme);
            }
            $(opt.target).append($mapPopover);
            if (opt.onload && $.isFunction(opt.onload)) {
                opt.onload(namespace);
            } else {
                $(document).trigger(opt.onload, [$mapPopover]);
            }
            var width = $mapPopover.width();
            var height = $mapPopover.height();
            var top = $this.position().top - height - 49;
            var left = $this.position().left - width / 2;

            if (opt.autoClose) {
                $(document).one("mousedown.mappopover", function(e) {
                    namespace.close();
                });
            }
            ...
            $mapPopover.mousedown(function() {
                return false;
            });
            $mapPopover.dblclick(function() {
                return false;
            });
        ...
        var namespace = {
            open: function() {
                if ($mapPopover && $mapPopover.remove) {
                    $mapPopover.remove();
                }
                initalPopover();
                var mapoverlay = $(opt.target).data('mapPin');
                if (opt.autoPosition && mapoverlay) {
                    mapoverlay.reviseMapCenterForPopover($mapPopover);
                }
                if (opt.onopen && $.isFunction(opt.onopen)) {
                    opt.onopen($this, $content);
                }
            },
            close: function() {
                if (opt.onclose && $.isFunction(opt.onclose)) {
                    opt.onclose($this);
                }
                $mapPopover.remove();
            },
        ...
        $this.data('mapPopover', namespace);
...
(function($) {
    $.fn.gautocomplete = function(option) {
        var defaultOption = {
            types: ['geocode'],
        ...
        var opt = $.extend({}, defaultOption, option);
        var gautocomplete = new google.maps.places.Autocomplete($(this).get(0), opt);

        $this.keydown(function() {
            $this.prop('matched', false);
        });
        google.maps.event.addListener(gautocomplete, 'place_changed', function() {
            $this.prop('matched', true);
            if (opt.onChange) {
                if ($.isFunction(opt.onChange)) {
                    opt.onChange($this);
                } else {
                    $(document).trigger(opt.onChange, [$this]);
                }
            }
        });
        ...
        google.maps.event.addDomListener($this.get(0), 'keyup', function(e) {
            if (e.keyCode === 27) {
                escPressed = true;
                return;
            }
            if (e.keyCode === 13) {
                if (escPressed) {
                    //let it go propagation while user last pressed key is esc
                    return;
                }
                //prevent further enter click from data-submit hook
                e.preventDefault();
                e.stopPropagation();
                return;
            }
        ...
        var namespace = {
            ...
            getValue: function() {
                var value = gautocomplete.getPlace();
                if (value) {
                    if (value.place_id) {
                        value.matched = $this.prop('matched');
                    } else {
                        value = {
                            matched: false
                        };
        ...
        $(this).data('gautoComplete', namespace);
        $this.attr('role', 'GAutocomplete');
    };
    var inital = function() {
        $('[data-gautocomplete]').each(function() {
            $(this).gautocomplete({
                onChange: $(this).attr('data-onChange')
            });
            $(this).attr('googleAutomcomplete', $(this).attr('data-gautocomplete'));
            $(this).removeAttr('data-gautocomplete');
        });
    };
    $(document).on('dom.load.gmap', function() {
        if ($('[data-gautocomplete]').length) {
            $.loadGMap(inital);
        }
    });
})(jQuery);
            ...
            geocoder.geocode({
                ...
                if (geostatus !== google.maps.GeocoderStatus.OK) {
                    if ($.isFunction(opt.error)) {
                        ...
                        $(document).trigger(opt.error, [geostatus]);
                    }
                } else {
                    if ($.isFunction(opt.success)) {
                        opt.success(results);
                    } else {
                        $(document).trigger(opt.success, [results]);
                    }
                }
            });
        };
...
(function($) {
    $.fn.mSlider = function(options) {
        var defaultOpts = {
            start: options.start || $(this).data("start") || 0,
            end: options.end || $(this).data("end") || 0,
            customSteps: options.customSteps || [],
            onSlide: options.onSlide,
            onCreate: options.onCreate
        };
        var opts = $.extend({}, defaultOpts, options);
        var start = opts.customSteps.indexOf(opts.start);
        var end = opts.customSteps.indexOf(opts.end);
        if (start >= 0 && end >= 0) {
            var result = [opts.customSteps[start], opts.customSteps[end]];
            $.fn.mSlider.getValues = function() {
                return result.map(function(item) {
                    return item == "No Min" || item == "No Max" ? "0" : item;
                });
            }
            return this.each(function() {
                var $this = $(this);
                $this.slider({
                    range: true,
                    min: 0,
                    max: opts.customSteps.length - 1,
                    values: [start, end],
                    step: 1,
                    slide: function(event, ui) {
                        result = [opts.customSteps[ui.values[0]], opts.customSteps[ui.values[1]]];
                        options.onSlide(result);
                    },
                    create: function() {
...
(function(root) {
	...
	var limit = 90;
	var nanobar = null
	root.progressbar = function(target) {
		if (bar) return bar;
		...
		nanobar = nanobar || new Nanobar(options);
		bar = {
			end: function() {
...
(function($) {
    $.fn.rangeSelect = function(options) {
        var defaultOpts = {
            ...
            customSteps: options.customSteps || [],
            maxShow: options.maxShow || 10,
            unitHeight: options.unitHeight || 37,
            duration: options.duration || 500,
            output: options.output || []
        };
        var opts = $.extend({}, defaultOpts, options);
        var arr = opts.customSteps;
        var arrR = reverseArr(arr);
        var priceFrom = opts.start;
        var priceTo = opts.end;
        var start = arr.indexOf(priceFrom);
        var end = arrR.indexOf(priceTo);
        var tempString = priceFrom;
        var isReboot = true;
        if (start >= 0 && end >= 0 && opts.maxShow > 0) {
            $.fn.rangeSelect.reboot = function($context) {
                if (isReboot) {
                    $context.find(".rangeSelect-right-wrapper .rangeSelect-up i, .rangeSelect-right-wrapper .rangeSelect-down i, .rangeSelect-right li").addClass("disabled");
                    $context.find(".rangeSelect-left-wrapper .rangeSelect-up i, .rangeSelect-left-wrapper .rangeSelect-down i, .rangeSelect-left li").removeClass("disabled");
                    isReboot = false;
                }
                $context.height((opts.maxShow + 2) * opts.unitHeight + 1);
                $context.finish().css("opacity", "0").slideDown().animate({
                    opacity: 1
                }, {
                    queue: false,
                    duration: 300
                });
                $context.find(".rangeSelect-left, .rangeSelect-left-wrapper, .rangeSelect-up, .rangeSelect-down").css("width", Math.round($context.width() / 2) + "px");
                $context.find(".rangeSelect-right, .rangeSelect-right-wrapper, .rangeSelect-down-right, .rangeSelect-up-right").css("width", ($context.width() - $context.find(".rangeSelect-left").width()) + "px");
                // $context.find(".rangeSelect-left-wrapper").show();
                // $context.find(".rangeSelect-right-wrapper").hide();
            }
            $.fn.rangeSelect.getValues = function() {
                return [priceFrom, priceTo];
            }
            return this.each(function() {
                var $this = $(this);
                $this.css("overflow", "hidden");
                $this.height((opts.maxShow + 2) * opts.unitHeight + 1);
                var ulMinWrapper = $("<div class='rangeSelect-left-wrapper'></div>");
                var ulMaxWrapper = $("<div class='rangeSelect-right-wrapper'></div>");
                var ulMin = $("<ul class='rangeSelect-left'></ul>");
                var ulMax = $("<ul class='rangeSelect-right'></ul>");
                for (var i = 0; i < arr.length; i++) {
                    ulMin.append("<li class='text-bold' data-value='" + arr[i] + "'>" + _formatPriceText(arr[i]) + "</li>");
                }
                ulMinWrapper.append(ulMin).append("<div class='rangeSelect-up'><i class='icon-caret-up'></i></div><div class='rangeSelect-down'><i class='icon-caret-down'></i></div>");
                for (var i = 0; i < arrR.length; i++) {
                    ulMax.append("<li class='text-bold' data-value='" + arrR[i] + "'>" + _formatPriceText(arrR[i]) + "</li>");
                }
                ulMaxWrapper.append(ulMax).append("<div class='rangeSelect-up rangeSelect-up-right'><i class='icon-caret-up'></i></div><div class='rangeSelect-down rangeSelect-down-right'><i class='icon-caret-down'></i></div>");
                $this.append(ulMinWrapper).append(ulMaxWrapper);
                $this.find(".rangeSelect-left, .rangeSelect-left-wrapper, .rangeSelect-up, .rangeSelect-down").css("width", Math.round($this.width() / 2) + "px");
                $this.find(".rangeSelect-right, .rangeSelect-right-wrapper, .rangeSelect-down-right, .rangeSelect-up-right").css("width", ($this.width() - $this.find(".rangeSelect-left").width()) + "px");
                $(".rangeSelect-left-wrapper, .rangeSelect-right-wrapper").css("margin", opts.unitHeight + "px 0");
                $(".rangeSelect-up, .rangeSelect-down, .rangeSelect-left li, .rangeSelect-right li").css({
                    height: opts.unitHeight + "px",
                    lineHeight: opts.unitHeight + "px"
                });
                $(".rangeSelect-up").height($(".rangeSelect-down").height() + 1);
                $(".rangeSelect-down").height($(".rangeSelect-down").height() + 2);
                $(".rangeSelect-up").css("top", -opts.unitHeight + "px");
                $(".rangeSelect-down").css("top", ($this.height() - opts.unitHeight * 2 - 2) + "px");
                $this.find(".rangeSelect-left li, .rangeSelect-right li").removeClass("active");
                $this.find(".rangeSelect-left li").eq(start).addClass("active");
                $this.find(".rangeSelect-right li").eq(end).addClass("active");
                $(opts.output[0]).text("$" + _formatPriceText(priceFrom));
                $(opts.output[1]).text(_formatPriceText(priceTo));
                $this.hide();
                $(document).on("click", ".rangeSelect-down", function() {
                    var h1 = parseInt($(this).siblings("ul").height());
                    var h2 = Math.abs(parseInt($(this).siblings("ul").css("top")));
                    var h3 = $this.height() - opts.unitHeight * 2;
                    if (h1 - h2 - h3 < h3) {
                        ...
                        $(this).siblings("ul").stop().animate({
                            top: "-=" + opts.unitHeight * opts.maxShow
                        }, opts.duration);
                    }
                });
                $(document).on("click", ".rangeSelect-up", function() {
                    var h1 = parseInt($(this).siblings("ul").height());
                    var h2 = Math.abs(parseInt($(this).siblings("ul").css("top")));
                    var h3 = $this.height();
                    if (h2 - h3 < 0) {
                ...
                $(document).on("click", ".rangeSelect-left li", function(e) {
                    // $(this).parents(".rangeSelect-left-wrapper").hide();
                    // $(".rangeSelect-right-wrapper").show();
                    // $(opts.output[0]).text($(this).text() === "No Min" ? $(this).text() : "$" + $(this).text());
                    if ($(this).hasClass("disabled")) {
                        ...
                        $(this).addClass("active");
                        priceFrom = $(this).data("value");
                        $this.find(".rangeSelect-right-wrapper .rangeSelect-up i, .rangeSelect-right-wrapper .rangeSelect-down i, .rangeSelect-right li").removeClass("disabled");
                        $this.find(".rangeSelect-left-wrapper .rangeSelect-up i, .rangeSelect-left-wrapper .rangeSelect-down i, .rangeSelect-left li").addClass("disabled");
                    }
                });
                $(document).on("click", ".rangeSelect-right li", function(e) {
                    if ($(this).hasClass("disabled")) {
                        e.preventDefault();
                    } else {
                        //$(this).parents(".rangeSelect-right-wrapper").hide();
                        $(opts.output[0]).text(tempString);
                        $(opts.output[1]).text($(this).text());
                        $(this).siblings().addBack().removeClass("active");
                        $(this).addClass("active");
                        priceTo = $(this).data("value");
                        $this.height(0);
                        isReboot = true;
                    }
            ...
            function _formatPriceText(val) {
                if (val == "No Min" || val == "No Max") {
                    return val;
                } else {
...
(function($) {
    $.rangeselector = function(opt) {
        var from = $('select[data-rangeselector=' + opt.key + '][data-from]');
        var to = $('select[data-rangeselector=' + opt.key + '][data-to]');
        if (!from || !to) return;

        var fdef = from.find('option[data-default]').val();
        var tdef = to.find('option[data-default]').val();

        function switchit(isfrom) {
            var fv = parseInt(from.val());
            var tv = parseInt(to.val());
            if (fv > tv) {
                isfrom ? to.val(tdef) : from.val(fdef);
            }
        }

        from.on('change', switchit.bind(null, true));
        to.on('change', switchit.bind(null, false));

        return [from, to];
    };

    $(document).on('click.rangeselector', '[data-rangeselector]', function() {
        var $this = $(this);
        var name = $this.data('rangeselector');
        var options = {key: name};
        var fromto = $.rangeselector(options);
        if (!fromto) return;
        fromto[0].removeAttr('data-rangeselector').removeAttr('data-from');
        fromto[1].removeAttr('data-rangeselector').removeAttr('data-to');
    });
})(jQuery);
...
//use it with slider control and lister onChange event
(function($) {
    $.fn.sliderthumbs = function(options) {
        ...
        var myclass = {
            listener: function(index) {
                ...
                var width = $item.outerWidth(true); //include the margin
                ...
                //if need to scroll
                if (left < 0 || left > pWidth - width) {
                    if (left < pWidth - left - width) {
                        targetScrollX = scrollLeft + left;
                    } else {
                        targetScrollX = scrollLeft - (pWidth - left - width);
                    }
        ...
        var opt = $.extend({}, myOptions, options);
        if (opt.listener) {
            $(document).on(opt.listener, function(e, i) {
                myclass.listener(i);
            });
        }

        $this.find('ul').on('keydown', function(e) {
            e.stopPropagation();
            e.preventDefault();
            ...
            if (e.keyCode === 39) {
                ...
                if (nextItem.length === 0) {
                    nextItem = $this.find('ul li').first();
                }
            } else if (e.keyCode === 37) {
                nextItem = currentItem.prev();
                if (nextItem.length === 0) {
                    nextItem = $this.find('ul li').last();
                }
            }
            nextItem[0].click();
        });
        $this.attr('role','SiliderThumbs');
        return myclass;
    };

    $(document).on('dom.load.sliderthumbs', function() {
        $('[data-sliderthumbs]').each(function() {
            var $this = $(this);
            var options = {
                listener: $this.attr('data-listener')
            }
            $this.sliderthumbs(options);
            $this.removeAttr('data-sliderthumbs');
        });
    });
})(jQuery);
...
//auto grow up when users type the words
(function($) {
    function typeWords() {
    ...
    $(document).on('click.typegrow', '[data-typegrow]', function() {
        var $o = $(this);
        $o.css('overflow', 'hidden');
        $o.on('input', typeWords);
        typeWords.call(this);
        $o.removeAttr("data-typegrow");
    });
})(jQuery);
